...
public class Map extends Activity {
	...
	private void updatePlaces() {
		...
		mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(lastLatLng, 14),
				1000, null);
		...
		protected String doInBackground(String... params) {
			...
			for (String placeSearchURL : params) {
				...
				try {
					...
					if (reponseCode.getStatusCode() == 200) { // 200 is code for
																// OK
			...
			try {
				...
				for (int j = 0; j < places.length; j++) {
					...
					try {
						...
						JSONObject placeLoc = placeObject.getJSONObject(
								"geometry").getJSONObject("location");

						minLat = (int) Math.min(
								placeLoc.getDouble("lat") * 1E6, minLat);
						minLong = (int) Math.min(
								placeLoc.getDouble("lng") * 1E6, minLong);
						maxLat = (int) Math.max(
								placeLoc.getDouble("lat") * 1E6, maxLat);
						maxLong = (int) Math.max(
								placeLoc.getDouble("lng") * 1E6, maxLong);
						pLatLng = new LatLng(Double.valueOf(placeLoc
								.getString("lat")), Double.valueOf(placeLoc
								.getString("lng")));
						...
						missingValue = true;
						Log.d("ERROR IN MAIN MAP CLASS",
								"Error occurred in parsing JSON + 1");
						e.printStackTrace();
					}
					if (missingValue) {
						places[j] = null; // if value is missing then no details
											// will be displayed
					} else {
				...
				Log.d("ERROR IN MAIN MAP CLASS",
						"Error occurred in parsing JSON + 2");
				e.printStackTrace();
			}
